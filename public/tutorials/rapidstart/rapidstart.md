...
Once you have DoneJS, you should have a folder with:
    ...
    can        - CanJS providing Model, View, Controls and routing
...
For this application, we will load some CanJS plugins. Add the following to __todos.js__:
...
objects with shared properties. It's used by __can.Construct__, __can.Observe__ and __can.Model__
(which extends can.Observe).

To create a __Construct__ of your own, call __can.Construct__ with the:
...
_Brief aside on `super`. You can use the [can.Construct.super] plugin call the function of the same name higher
...
### constructor / init `new Construct(arg1, arg2)`

When a class constructor is invoked, __can.Construct__ creates the instance and 
calls [can.Construct.prototype.init] with 
the arguments passed to `new Construct(...)`.
...
Model uses static [can.Model.findAll findAll], [can.Model.findAll findOne], [can.Model.create create], [can.Model.update update], and [can.Model.destroy destroy] methods to create, read, update and delete  model instances on the server.  

Now you can call methods on Todo that make changes on the server.  For example,  in your console, try:

    Todo.findAll({});

In the console, you'll see it make a request  to `GET /todos`.

Assuming your server does not have a `/todos` service, this won't work.  That's ok, we can simulate them with [can.fixture].
...
[can.Model::elements Elements] retrieves the elements that 
...
## Control `can.Control(name, classProps, prototypeProps)`
...
into higher-order business rules with [can.route]. Its serves as 
...
[can.Control.prototype.element this.element] is the jQuery-wrapped

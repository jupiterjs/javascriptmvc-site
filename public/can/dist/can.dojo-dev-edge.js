...
;
    ...
    define("plugd/trigger", ["dojo"], function (dojo) {
        var d = dojo, isfn = d.isFunction, leaveRe = /mouse(enter|leave)/, _fix = function (_, p) {
            return "mouse" + (p == "enter" ? "over" : "out");
        }, mix = d._mixin, realTrigger = d.doc.createEvent ? function (n, e, a) {
        ...
        if (d._Node && !d._Node.prototype.trigger) {
            d.extend(d._Node, {trigger:function (ev, data) {
    ...
    var id = 0, addBinding = function (nodelist, ev, cb) {
        ...
        nodelist.forEach(function (node) {
            var node = new dojo.NodeList(node), events = can.data(node, "events"), handlers = events[ev], handler = handlers[cb.__bindingsIds];
    ...
    can.ajax = function (options) {
        ...
        var def = method({url:options.url, handleAs:options.dataType, sync:!options.async, headers:options.headers, content:options.data});
    ...
    can.buildFragment = function (frags, nodes) {
        var owner = nodes.length && nodes[0].ownerDocument, frag = dojo.toDom(frags[0], owner);
        if (frag.nodeType !== 11) {

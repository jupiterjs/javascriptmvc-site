...
0!==b?La:R)(i);if(0==b)return"<"+a+c.view.hook(e?function(a){var b=a.parentNode,c=document.createTextNode(i),e=function(){var a=C(f,d);c.nodeValue=""+a.value;t(a.observed,b,e,j)};b.insertBefore(c,a);b.removeChild(a);t(h,b,e,j)}:function(a){var b=function(a,b){var d=c.view.frag(a),f=c.map(d.childNodes,function(a){return a}),e=b[b.length-1];e.nextSibling?e.parentNode.insertBefore(d,e.nextSibling):e.parentNode.appendChild(d);c.remove(c.$(b));return f},e=b(i,[a]),g=function(){var c=C(f,d);e=b(c.value,
...
var Na=RegExp("(<%%|%%>|<%==|<%=|<%#|<%|%>|<|>|\"|')","g"),z=null,F=q=null,y=[],Ma=function(a,b){var d=[],c=0,a=a.replace(Ia,"\n");a.replace(Na,function(b,e,g){g>c&&d.push(a.substring(c,g));d.push(e);c=g+e.length});0===c&&d.push(a);var e="",g=["var ___v1ew = [];"],h=function(a,b){g.push("___v1ew.push(",'"',a.split("\\").join("\\\\").split("\n").join("\\n").split('"').join('\\"').split("\t").join("\\t"),'"'+(b||"")+");")},i=[],j,k=null,l=!1,n="",o=0,p;for(z=q=F=null;(p=d[o++])!==m;){if(null===k)switch(p){case "<%":case "<%=":case "<%==":l=
1;case "<%#":k=p;e.length&&h(e);e="";break;case "<%%":e+="<%";break;case "<":0!==d[o].indexOf("!--")&&(z=1,l=0);e+=p;break;case ">":z=0;l?(h(e,',can.EJS.pending(),">"'),e=""):e+=p;break;case "'":case '"':z&&(q&&q===p?q=null:null===q&&(q=p,F=j));default:"<"===j&&(n=p.split(" ")[0]),e+=p}else switch(p){case "%>":switch(k){case "<%":j=--e.split("{").length- --e.split("}").length;1==j?(g.push("___v1ew.push(","can.EJS.txt('"+n+"',"+(q?"'"+F.match(ma)[1]+"'":z?1:0)+",this,function(){","var ___v1ew = [];",
e),i.push({before:"",after:"return ___v1ew.join('')}));"})):(c=i.length&&-1==j?i.pop():{after:";"},c.before&&g.push(c.before),g.push(e,";",c.after));break;case "<%=":case "<%==":(j=--e.split("{").length- --e.split("}").length)&&i.push({before:"return ___v1ew.join('')",after:"}));"}),la.test(e)&&(e=e.match(la),e="function(__){var "+e[1]+"=can.$(__);"+e[2]+"}"),g.push("___v1ew.push(","can.EJS."+("<%="===k?"esc":"txt")+"('"+n+"',"+(q?"'"+F.match(ma)[1]+"'":z?1:0)+",this,function(){ return ",e,j?"var ___v1ew = [];":

...
if(this==document.documentElement)return this.ret.push(this);var j=e?jQuery.data(this,"offset")||jQuery.data(this,"offset",g.offset()):g.offset(),i=g.width();g=g.height();(res=!(j.top>a+d||j.top+g<a||j.left>h+b||j.left+i<h))&&f.push(this)});return this.pushStack(jQuery.unique(f),"withinBox",jQuery.makeArray(arguments).join(","))}})(jQuery);
...
b,d);if(!f)return}for(g=0;g<e.length;g++){this.move(e[g],b,d);if(!f)return}}},end:function(a,b){for(var d,e=this.lowerName+"end",f=0;f<this.last_active.length;f++){d=this.last_active[f];this.isAffected(a.vector(),b,d)&&d[this.endName]&&d.callHandlers(this.endName,null,a,b)}for(f=0;f<this._elements.length;f++)(d=c.data(this._elements[f],"_dropData"))&&d.callHandlers(e,null,a,b);this.clear()},clear:function(){this._elements.each(function(){c.removeData(this,"_dropData")});this._elements=c();delete this.dragging}});

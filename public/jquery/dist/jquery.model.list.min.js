(function(e){var k={undHash:/_|-/,colons:/::/,words:/([A-Z]+)([A-Z][a-z])/g,lowUp:/([a-z\d])([A-Z])/g,dash:/([a-z\d])([A-Z])/g,replacer:/\{([^\}]+)\}/g,dot:/\./},q=function(c,g,h){return c[g]!==undefined?c[g]:h&&(c[g]={})},p=function(c){return(c=typeof c)&&(c=="function"||c=="object")},o=function(c,g,h){c=c?c.split(k.dot):[];var a=c.length;g=e.isArray(g)?g:[g||window];var b,d,f,i=0;if(a==0)return g[0];for(;b=g[i++];){for(f=0;f<a-1&&p(b);f++)b=q(b,c[f],h);if(p(b)){d=q(b,c[f],h);if(d!==undefined){h===
false&&delete b[c[f]];return d}}}},n=e.String=e.extend(e.String||{},{getObject:o,capitalize:function(c){return c.charAt(0).toUpperCase()+c.substr(1)},camelize:function(c){c=n.classize(c);return c.charAt(0).toLowerCase()+c.substr(1)},classize:function(c,g){c=c.split(k.undHash);for(var h=0;h<c.length;h++)c[h]=n.capitalize(c[h]);return c.join(g||"")},niceName:function(c){return n.classize(c," ")},underscore:function(c){return c.replace(k.colons,"/").replace(k.words,"$1_$2").replace(k.lowUp,"$1_$2").replace(k.dash,
"_").toLowerCase()},sub:function(c,g,h){var a=[];a.push(c.replace(k.replacer,function(b,d){b=o(d,g,typeof h=="boolean"?!h:h);d=typeof b;if((d==="object"||d==="function")&&d!==null){a.push(b);return""}else return""+b}));return a.length<=1?a[0]:a},_regs:k})})(jQuery);

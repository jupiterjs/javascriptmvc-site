...
j.renderer(a,i);if(k.cache)k.cached[a]=i;return{view:i}};if(m=document.getElementById(f))c=m.type.match(/\/[\d\w]+$/)[0].replace(/^\//,".");if(!c){c=k.ext;f+=k.ext}a=w(f);if(f.match(/^\/\//)){var d=f.substr(2);f=typeof steal==="undefined"?(f="/"+d):steal.root.mapJoin(d)}j=k.types[c];return{send:function(i,h){if(k.cached[a])return h(200,"success",{view:k.cached[a]});else if(m)h(200,"success",e(m.innerHTML));else b=n.ajax({async:g.async,url:f,dataType:"text",error:function(){z("",f);h(404)},success:function(l){z(l,
f);h(200,"success",e(l))}})},abort:function(){b&&b.abort()}}});n.extend(k,{hookups:{},hookup:function(c){var g=++F;k.hookups[g]=c;return g},cached:{},cache:true,register:function(c){this.types["."+c.suffix]=c;window.steal&&steal.type(c.suffix+" view js",function(g,f,j){f=k.types["."+g.type];var m=w(g.rootSrc);g.text=f.script(m,g.text);j()})},types:{},ext:".ejs",registerScript:function(c,g,f){return"$.View.preload('"+g+"',"+k.types["."+c].script(g,f)+");"},preload:function(c,g){k.cached[c]=function(f,
j){return g.call(f,f,j)}}});window.steal&&steal.type("view js",function(c,g,f){g=k.types["."+c.type];var j=w(c.rootSrc);c.text="steal('"+(g.plugin||"jquery/view/"+c.type)+"').then(function($){$.View.preload('"+j+"',"+c.text+");\n})";f()});var I,x,J,C,u,D,q,A={val:true,text:true};I=function(c){var g=n.fn[c];n.fn[c]=function(){var f=y(arguments),j,m,a=this;if(t(f[0])){f[0].done(function(b){x.call(a,[b],g)});return this}else if(J(f)){if(j=D(f)){m=f[j];f[j]=function(b){x.call(a,[b],g);m.call(a,b)};k.apply(k,
